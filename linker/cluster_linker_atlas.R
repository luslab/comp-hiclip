...
structures.clusters.dt <- merge(clusters.dt, structure.dt, by = "name", all.x = TRUE)
shuffled.clusters.dt <- merge(structures.clusters.dt, shuffled.dt, by = "name", all.x = TRUE)
fwrite(shuffled.clusters.dt, paste0("all", ".", "atlas", ".clustered.tsv.gz"), sep = "\t")