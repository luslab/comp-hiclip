...
```{r}
# Shuffled control MEA functions for non-hybrids

run_rnafold <- function(sequence, args =  c("-p", "--noLP", "--noPS", "--MEA", sequence)) {
  
  rnafold.out <- system2(command = "RNAfold", args = args, stdout = TRUE)
  return(rnafold.out)
  
}


extract_mea_fe <- function(mea) {
  
  mea_fe <- str_split(mea, "\\{")[[1]][[2]]
  mea_fe <- str_remove_all(mea_fe, " ")
  mea_fe <- as.numeric(str_split(mea_fe, "MEA")[[1]][[1]])
  return(mea_fe)
  
}


get_shuffled_mea <- function(id, sequence) {
  ...
  shuff_fasta <- sapply(1:ncol(shuffled), function(i) RNAStringSet(shuffled[,i]))
  shuff_fasta <- unlist(RNAStringSetList(shuff_fasta))
  
  #names(shuff_fasta) <- sapply(1:length(shuff_fasta), function(i) paste0(id, "_",i))
  
  temp.fasta <- paste0(id,".fa")
  writeXStringSet(shuff_fasta, filepath = paste0(id,".fa"))
  
  rnafold.out <- run_rnafold(temp.fasta)
  
  rnafold.out.mea <- rnafold.out[str_detect(rnafold.out, "MEA")]
  rnafold.out.mea_fe <- lapply(rnafold.out.mea, function(x) extract_mea_fe(x))
  rnafold.out.mea_fe <- rnafold.out.mea_fe[!is.na(rnafold.out.mea_fe)]
  rnafold.out.mea_fe.mean <- mean(unlist(rnafold.out.mea_fe))
  
  #names(rnafold.out.mea_fe.mean) <- id
 
  invisible(file.remove(temp.fasta))
  return(list(id = id, shuffled_mea_fe = rnafold.out.mea_fe.mean))
...
nonhybrids.mfe.df <- nonhybrids.mfe.df %>%
  dplyr::select(id, stem_loop_seq, stem_loop_db, eval_mfe) %>%
  distinct()
...
# fwrite(nonhybrids.mfe.df, "rnafold_test.txt", sep = "\t")
nonhybrids.shuff.mfe.df <- nonhybrids.mfe.df %>%
  rowwise() %>%
  mutate(mea = list(get_shuffled_mea(id, stem_loop_seq))) %>%
  mutate(shuffled_mea_fe = mea$shuffled_mea_fe) %>%
  dplyr::select(-mea) %>%
  ungroup() %>%
  dplyr::select(id, shuffled_mea_fe)

nornase.forgi.dt <- left_join(nornase.forgi.dt, nonhybrids.shuff.mfe.df, by = "id")

nonhybrids.shuff.mfe.df <- mutate(nonhybrids.shuff.mfe.df, Sample = "shuffled_mfe", Experiment = "STAU1\nnonhybrids")

nonhybrids.shuff.mfe.df <- dplyr::rename(nonhybrids.shuff.mfe.df, MFE = shuffled_mea_fe)
```
...
```{r}
nonhybrids.mfe.df <- rbind(nonhybrids.mfe.df, nonhybrids.shuff.mfe.df)
```
...
```{r}
filtered.nonhyb.mfe.df <- semi_join(nonhybrids.mfe.df, filtered.nonhyb.forgi.df, by = "id")
```
...
x <- nolinker_nuc_freq.df %>% dplyr::select(id, A_freq_max_pur_arm, G_freq_max_pur_arm, C_freq_max_pur_arm, U_freq_max_pur_arm,A_freq_min_pur_arm, G_freq_min_pur_arm, C_freq_min_pur_arm, U_freq_min_pur_arm)
    dplyr::distinct() %>%
