...
```{r}
# plot.path <- "~/Dropbox (The Francis Crick)/comp_hiclip/plots/figure_5_with_none/"
# plot.path <- "~/Dropbox (The Francis Crick)/comp_hiclip/plots/figure_5_redo"
...
gt_colours <- c("None" = "#4f6980", "None\n(Expression\nMatched)" = "#4f6980", "PARIS" = "#849db1", "STAU1" = "#a2ceaa", "STAU1 & PARIS" = "#638b66") # Miller Stone
...
if(!interactive()) ggsave(p, filename = file.path(plot.path, "rna_abundance.pdf"), width = 5, height = 4)
# if(interactive()) ggsave(p, filename = file.path(plot.path, "rna_abundance_expression_matched.pdf"), width = 5, height = 4)
# if(interactive()) ggsave(p, filename = file.path(plot.path, "rna_abundance_paris_expression_matched.pdf"), width = 5, height = 4)
p
```
...
if(!interactive()) ggsave(p, filename = file.path(plot.path, "rna_localisation.pdf"), width = 5, height = 4)
p
```
...
if(!interactive()) ggsave(p, filename = file.path(plot.path, "rna_translation.pdf"), width = 5, height = 4)
p
```
...
# q <- quantile(gene.counts.dt[stau %in% "STAU1"]$mean_count, probs = seq(0, 1, 0.05))
# gene.counts.dt[stau %in% "PARIS" & mean_count > q[2], stau := "PARIS Expresssion Matched"]
...
# if(!interactive()) ggsave(p, filename = file.path(plot.path, "rna_metabolism.pdf"), width = 9, height = 6)
if(interactive()) ggsave(p, filename = file.path(plot.path, "rna_metabolism_expression_matched.pdf"), width = 9, height = 6)
# if(interactive()) ggsave(p, filename = file.path(plot.path, "rna_metabolism_paris_expression_matched.pdf"), width = 9, height = 6)
p
```
...
```{r}
kruskal.test(x = utr3.duplexes.dt$utr3_width, 
               g = utr3.duplexes.dt$metabolism_cluster)

d <- dunn.test(x = utr3.duplexes.dt$utr3_width, 
               g = utr3.duplexes.dt$metabolism_cluster,
               method = "bh",
               alpha = 0.01)
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "utr3_length.pdf"), width = 4, height = 4)
p
```
...
```{r, eval = FALSE}
# kruskal.test(x = utr3.duplexes.dt$loop, 
#                g = utr3.duplexes.dt$metabolism_cluster)
# 
...
#        fill = "")
# 
# if(interactive()) ggsave(p, filename = file.path(plot.path, "duplex_span.pdf"), width = 4, height = 4)
# p
```
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "duplex_span_density.pdf"), width = 4, height = 8)
p
```
...
```{r}
kruskal.test(x = utr3.duplexes.dt$circ_score, 
               g = utr3.duplexes.dt$metabolism_cluster)

d <- dunn.test(x = utr3.duplexes.dt$circ_score, 
               g = utr3.duplexes.dt$metabolism_cluster,
               method = "bh",
               alpha = 0.01)
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "circ_score.pdf"), width = 4, height = 4)
p
```
<!-- ## Relative starts -->
...
<!-- if(interactive()) ggsave(p, filename = file.path(plot.path, "relative_arm_location_start_start.pdf"), width = 5, height = 8) -->
...
<!-- p.l <- ggplot(utr3.duplexes.melted.dt[variable == "L_abs_start"], aes(x = value, y = ..density..)) + -->
...
<!--   labs(x = "Distance from 3' UTR start/end", -->
<!--        y = "Density") + -->
<!--   coord_cartesian(xlim = c(-1000, 10000), ylim = c(0, 1e-3)) -->

<!-- p.r <- ggplot(utr3.duplexes.melted.dt[variable == "R_abs_end"], aes(x = value, y = ..density..)) + -->
...
<!--   labs(x = "", -->
<!--      y = "") + -->
<!--   coord_cartesian(xlim = c(-10000, 1000), ylim = c(0, 1e-3)) -->

<!-- if(interactive()) ggsave(p.l + p.r, filename = file.path(plot.path, "absolute_arm_locations_histogram.pdf"), width = 6, height = 4) -->
<!-- p.l + p.r -->
<!-- ``` -->
...
<!-- if(!interactive()) ggsave(p, filename = file.path(plot.path, "degradation_arm_distance.pdf"), width = 4, height = 4) -->
<!-- p -->
<!-- ``` -->
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "mean_utr3_length.pdf"), width = 4, height = 4)
p
```
## Loop length
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "mean_duplex_span_density.pdf"), width = 4, height = 8)
p
```
...
if(interactive()) ggsave(p, filename = file.path(plot.path, "mean_circ_score.pdf"), width = 4, height = 4)
p
```
# Bin rel starts
...
longest.duplexes.dt[0 <= L_rel_start & L_rel_start < 0.25, L_rel_q := 1]
longest.duplexes.dt[0.25 <= L_rel_start & L_rel_start < 0.5, L_rel_q := 2]
longest.duplexes.dt[0.5 <= L_rel_start & L_rel_start < 0.75, L_rel_q := 3]
longest.duplexes.dt[0.75 <= L_rel_start & L_rel_start <= 1, L_rel_q := 4]
longest.duplexes.dt[L_rel_start < 0, L_rel_q := 1]

longest.duplexes.dt[0 <= R_rel_start & R_rel_start < 0.25, R_rel_q := 1]
longest.duplexes.dt[0.25 <= R_rel_start & R_rel_start < 0.5, R_rel_q := 2]
longest.duplexes.dt[0.5 <= R_rel_start & R_rel_start < 0.75, R_rel_q := 3]
longest.duplexes.dt[0.75 <= R_rel_start & R_rel_start <= 1, R_rel_q := 4]
...
longest.duplexes.dt[rel_q %in% c("1-1", "2-2", "3-3", "4-4"), q_span := "1Q"]
longest.duplexes.dt[rel_q %in% c("1-2", "2-3", "3-4"), q_span := "2Q"]
longest.duplexes.dt[rel_q %in% c("1-3", "2-4"), q_span := "3Q"]
longest.duplexes.dt[rel_q %in% c("1-4"), q_span := "4Q"]

longest.duplexes.dt[, loop_g := ifelse(loop < 100, "< 100", ">= 100")]

p <- ggplot(longest.duplexes.dt, aes(x = rel_q, fill = metabolism_cluster)) +
  geom_bar() +
  scale_fill_manual(values = metabolism_colours) +
  geom_vline(xintercept = c(4.5, 7.5, 9.5), linetype = "dotted") +
  # facet_grid(loop_g ~ .) +
...
# if(interactive()) ggsave(p, filename = file.path(plot.path, "rel_q_bar.pdf"), width = 6, height = 4)
p
```

```{r}
p1 <- ggplot(longest.duplexes.dt, aes(x = rel_q)) +
  geom_bar() +
  geom_vline(xintercept = c(4.5, 7.5, 9.5), linetype = "dotted") +
  # facet_grid(loop_g ~ .) +
  theme_minimal_grid() + theme(legend.position = "bottom") +
...
p2 <- ggplot(longest.duplexes.dt, aes(x = rel_q, fill = metabolism_cluster)) +
  geom_bar(position = "fill") +
  # facet_grid(loop_g ~ .) +
  # facet_grid(. ~ q_span, scales = "free_x") +
  geom_vline(xintercept = c(4.5, 7.5, 9.5), linetype = "dotted") +
  scale_fill_manual(values = metabolism_colours) +
  theme_minimal_grid() + theme(legend.position = "bottom") +
  labs(x = "Duplex regions",
...
# if(interactive()) ggsave(p, filename = file.path(plot.path, "rel_q_fill.pdf"), width = 6, height = 4)
if(interactive()) ggsave(plot_grid(p1, p2, ncol = 1, rel_heights = c(1, 2)), filename = file.path(plot.path, "quarters.pdf"), width = 6, height = 4)

p
```

```{r}
p <- ggplot(longest.duplexes.dt, aes(x = q_span, fill = metabolism_cluster)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = metabolism_colours) +
  theme_minimal_grid() + theme(legend.position = "bottom") +
  labs(x = "Duplex regions",
...
```{r}
p <- ggplot(longest.duplexes.dt, aes(x = rel_q, y = Degradation)) +
  # geom_violin() +
  geom_boxplot(width = 0.5, outlier.shape = NA) +
  scale_y_log10() +
  annotation_logticks(sides = "l") +
  coord_cartesian(y = c(0.0005, 0.05)) +
  scale_fill_manual(values = metabolism_colours) +
  theme_minimal_grid() + theme(legend.position = "bottom") +
  labs(x = "Duplex regions",
       y = "Count",
