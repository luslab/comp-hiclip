...
```{r}
stau1.hybrids.dt <- fread("/Volumes/lab-luscomben/home/shared/projects/ira-nobby/comp_hiclip/merged_clusters/stau1_atlas/merged_atlas.clusters.tsv.gz")
```
...
stau1.cluster.hybrids.df <- stau1.cluster.hybrids.df %>%
  rowwise() %>%
  mutate(name = paste(cluster, L_seqnames, R_seqnames, sep = "-")) %>%
  ungroup()

stopifnot(length(unique(stau1.cluster.hybrids.df$name)) == nrow(stau1.dt))
```
...
stau1.samples.df <- stau1.cluster.hybrids.df %>%
