...
```{r}
# Clusters (collapsed)
# stau1.dt <- fread(paste0(data.dir, "/stau1_atlas/merged_atlas.clusters.collapsed.mfe.tsv.gz"))
stau1.dt <- fread(paste0(data.dir, "/stau1_atlas/merged_atlas.clusters.collapsed_plus_nonhybrids.mfe.tsv.gz"))
...
stau1_forgi.dt <- fread(paste0(data.dir,"/stau1_atlas/merged_atlas.clusters.collapsed_plus_nonhybrids.forgi.tsv.gz"))
...
```{r}
nrow(stau1.dt)
nrow(paris.dt) # includes tRNA and rRNA
```

```{r}
# Exclude rRNA and tRNA
...
reg.gg <- plot_stacked_barchart(stau1.region.df, region, simplify = FALSE) +
  theme_minimal_grid() + 
...
reg.gg
# ggsave(paste0(plots.dir, "/stau1_arm_regions.pdf"), reg.gg, width = 6, height = 4, dpi = 300)
```
...
ii.gg <- plot_stacked_barchart(stau1.dt, column = type, simplify = FALSE) +
...
# ggsave(paste0(plots.dir, "/stau1_intra_inter.pdf"), ii.gg, width = 6, height = 4, dpi = 300)
...
#### Intramolecular locations

```{r warning=FALSE}
# Filter out duplexes with no structure
...
```{r}
interregion_colours <- c("CDS - CDS" = "#AF7AC5", "CDS - ncRNA" = "#B03A2E", "CDS - UTR3" = "#F1948A",
                    "CDS - UTR5" = "#E69F00", "ncRNA - ncRNA" = "#999999", "Other" = "#17A589",
                    "UTR3 - UTR3" = "#56B4E9", "UTR5 - UTR5" = "#2874A6")
```
...
The are `r nrow(intra_paris.dt)` intramolecular PARIS duplexes, excluding introns. 
...
paris_regions.gg
# ggsave(paste0(data.dir, "paris_region_pairs.pdf"), paris_regions.gg, width = 5, height = 8, dpi = 300)
```
...
#### Need to adjust the genomic coordinates to focus on the stem of the structure


```{r}
utr3_clusters.df <- trim_duplex_overhangs(utr3_clusters.df)
```
...
#   rowwise() %>%
#   mutate(duplex_span1 = R_start - L_end - 1) %>%
#   ungroup()
...
duplex_span.gg
#ggsave(paste0(plots.dir, "/duplex_span.pdf"), duplex_span.gg, dpi = 300, width = 4, height = 4)
```
### Span groups

```{r}
utr3_clusters.df <- utr3_clusters.df %>%
  dplyr::mutate(span_group = case_when(duplex_span < 20 ~ "short-range",
                                       duplex_span >= 20 & duplex_span <= 100 ~ "medium-range",
                                       duplex_span > 100 ~ "long-range"))

plot_stacked_barchart(data.df = utr3_clusters.df, column = span_group, group = Experiment, simplify = FALSE)
```
...
set.seed(42)
mixmdl <- normalmixEM(log10(utr3_clusters.df[utr3_clusters.df$Experiment == "STAU1", ]$duplex_span), k = 2)
...
```{r}
# Wilcox.test
stau1.bp.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$total_paired
paris.bp.ls <- dplyr::filter(utr3_clusters.df, Experiment == "PARIS")$total_paired
...
total_paired.gg <- ggplot(utr3_clusters.df,
  ...
  annotate("text", label = paste0("p < 2.2e-16"), x = 1.5, y = 80)
total_paired.gg

# ggsave(paste0(plots.dir, "/total_paired.pdf"), total_paired.gg, dpi = 300, width = 4, height = 6)
```

### Separate by the range of the interaction

```{r}
total_paired2.gg <- ggplot(utr3_clusters.df,
                         aes(x = Experiment, y=total_paired, fill=Experiment)) +
  geom_boxplot() +
  theme(legend.position="right", legend.direction="vertical") +
  scale_fill_tableau() +
  ...
  theme(legend.position = "top") +
  ylab("Base-pairs per duplex (bp)") +
  facet_grid(cols = vars(span_group))
total_paired2.gg 
```
...
```{r}
# Test
stau1.mfe.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$mfe
stau1.norm_mfe.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$normalised_mfe
stau1.shuffled.mfe.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$normalised_mean_shuffled_mfe
...
```{r}
stau1.paired_ratio.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$paired_percentage
...
bulges.gg <- ggplot(utr3_clusters.df,
  ...
  geom_label(data = utr3_counts.df, aes(label=paste0("n = ",counts), y = -1.5), label.padding = unit(0.25, "lines"),
            position=position_dodge(width=1.0), inherit.aes = TRUE,  fill = "white", size = 6)
bulges.gg
# ggsave(paste0(plots.dir, "/bulge_count.pdf"), bulges.gg, dpi = 300, width = 4, height = 6)
```


```{r}
bulges2.gg <- ggplot(utr3_clusters.df,
                         aes(x = Experiment, y=bulge_count, fill=Experiment)) +
  geom_boxplot() +
  scale_fill_brewer(palette="Blues") +
  theme(legend.position="right", legend.direction="vertical") +
...
symmetry.gg <- plot_stacked_barchart(symmetry.df, column = symmetry_type, group = Experiment, simplify = FALSE) +
  ...
  guides(fill = guide_legend(title = "Structure type")) +
    theme_void()
symmetry.gg 
# ggsave(paste0(plots.dir, "/symmetry.pdf"), symmetry.gg, dpi = 300, width = 7, height = 4)
```
```{r}
...
symmetry2.gg <- plot_stacked_barchart(symmetry2.df, column = symmetry_type, group = Experiment, simplify = FALSE) +
  scale_fill_brewer(palette = "PuBuGn") +
  coord_polar("y", start=0) +
  guides(fill = guide_legend(title = "Structure type")) +
    theme_void()
...
How many clusters contain hybrids from same sample or different samples?

```{r}
 stau1.cluster.hybrids_summary.df <- stau1.cluster.hybrids.df %>%
  group_by(id) %>%
  #mutate(sample_set = list(sort(unique(sample)))) %>%
...
table(stau1.samples.df$sample_set)
```

Focus on 3'UTR-3'UTR intramolecular interactions only:

```{r}
stau1.samples.df$Experiment <- "STAU1"
utr3_clusters.df <- left_join(utr3_clusters.df, dplyr::select(stau1.samples.df, Experiment, id, sample_set), by = c("Experiment", c("name" = "id")))
```


```{r}
source_counts.df <- utr3_clusters.df %>%
  dplyr::filter(Experiment == "STAU1") %>%
  dplyr::select(name, Experiment, span_group, sample_set) %>%
  distinct() %>%
    group_by(sample_set) %>%
    summarise(counts = n()) 
...
```{r}
source.upset.input <- c("stau1_linker" = source_counts.df[source_counts.df$sample_set == "stau1_linker",]$counts[1],
                        "stau1_nolinker" = source_counts.df[source_counts.df$sample_set == "stau1_nolinker",]$counts[1],
                        "stau1_nonhybrid" = source_counts.df[source_counts.df$sample_set == "stau1_nonhybrid",]$counts[1],
               "stau1_linker&stau1_nolinker" = source_counts.df[source_counts.df$sample_set == "stau1_linker,stau1_nolinker",]$counts[1],
               "stau1_linker&stau1_nonhybrid" = source_counts.df[source_counts.df$sample_set == "stau1_linker,stau1_nonhybrid",]$counts[1],
               "stau1_nolinker&stau1_nonhybrid" = source_counts.df[source_counts.df$sample_set == "stau1_nolinker,stau1_nonhybrid",]$counts[1],
               "stau1_linker&stau1_nolinker&stau1_nonhybrid" = source_counts.df[source_counts.df$sample_set == "stau1_linker,stau1_nolinker,stau1_nonhybrid",]$counts[1])
...
Overlap with PARIS for 3'UTR intramolecular duplexes

```{r}
stau1.utr3.dt <- data.table(stau1.utr3.df)
...
```{r}
#pdf(file.path(plots.dir, "stau1_paris_upset.pdf"), width = 7, height = 5)
