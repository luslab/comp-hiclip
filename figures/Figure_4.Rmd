...
stau1_duplex_regions.df <- intra_stau1.dt %>%
  dplyr::filter(!str_detect(cluster, "ID")) %>%
  rowwise() %>%
...
Overlap with PARIS for 3'UTR intra-transcript duplexes

```{r}
# Keep only proximity ligation hybrids
exp_stau1.utr3.df <- stau1.utr3.df %>%
  dplyr::filter(!str_detect(cluster, "ID"))
...
```{r}
# stau1.utr3.dt <- data.table(dplyr::filter(stau1.utr3.df, !str_detect(cluster, "ID")))
# paris.utr3.dt <- data.table(paris.utr3.df)
# 
# stau1.utr3.dt <- reorient_hybrids(stau1.utr3.dt)
# paris.utr3.dt <- reorient_hybrids(paris.utr3.dt)
...
#### Calculate hybrid spans to account for introns


```{r}
get_loop_size <- function(utr3.duplexes.dt, utr3.grl) {
...
```{r}
utr3_paris.dt <- data.table(dplyr::filter(utr3_clusters.df, Experiment == "PARIS" & detection_type == "Proximity ligation"))
utr3_stau1.dt <- data.table(dplyr::filter(utr3_clusters.df, Experiment == "STAU1"))

utr3_stau1.dt[, ensg := sapply(strsplit(L_gene_id, "\\."), "[[", 1)]
utr3_paris.dt[, ensg := sapply(strsplit(L_gene_id, "\\."), "[[", 1)]
```


```{r}
utr3_stau1.dt <- get_loop_size(utr3_stau1.dt, utr3.grl)
# utr3_paris.dt <- get_loop_size(utr3_paris.dt, utr3.grl)



# nrow(rbind(utr3_stau1.dt, utr3_paris.dt))

```

```{r}
#fwrite(utr3_stau1.dt, file = "~/Desktop/stau1_utr3_loop_lengths.tsv.gz", sep = "\t")
```
...
```{r}
utr3_paris.dt <- utr3_paris.dt[L_seqnames == R_seqnames][L_region == "UTR3"][R_region == "UTR3"] # To check all are intra 3' UTR
...
```{r}
test <- dplyr::filter(utr3_paris.dt, str_detect(name, "C001-ZNF652"))
```
...
```{r}
utr3.duplexes.dt <- utr3_paris.dt
```

```{r}
utr3.duplexes.dt <- utr3.duplexes.dt[1:10, ]
```

```{r}
utr3.duplexes.dt <- rbind(utr3.duplexes.dt, test)
```


```{r}
opts=c('exon_id','exon_name','exon_rank') 

tp=list(integer(),character(),integer())
...
```{r}
utr3.dt <- rbindlist(utr3.dt, fill = TRUE)
```
...
```{r}
span_median <- utr3_stau1.dt %>%
    group_by(Experiment, detection_type) %>%
    dplyr::summarize(Median = median(loop, na.rm=TRUE))
span_median

span_mean <- utr3_stau1.dt %>%
    group_by(Experiment, detection_type) %>%
    dplyr::summarize(Mean = mean(loop, na.rm=TRUE))
span_mean
```


```{r}
duplex_span.gg <- ggplot(utr3_stau1.dt, aes(x = loop, color = Experiment)) + 
  ...
  geom_vline(data=span_median, aes(xintercept=Median, color=Experiment),
             linetype="dashed") +
  facet_grid(rows = vars(detection_type), scales = "free_y")
...
```{r}
utr3_stau1.dt[utr3_stau1.dt$Experiment == "STAU1" & utr3_stau1.dt$detection_type == "Proximity ligation" & utr3_stau1.dt$loop == 0 ,]
```
...
set.seed(42)
mixmdl <- normalmixEM(log10(utr3_stau1.dt[utr3_stau1.dt$Experiment == "STAU1" & utr3_stau1.dt$detection_type == "Proximity ligation" & utr3_stau1.dt$loop > 0 ,]$loop), k = 2)
...
### Filter out predicted structures with stems shorter than 8 bp

```{r}
# utr3_clusters.df <- utr3_clusters.df %>%
#   dplyr::filter(!(str_detect(cluster, "ID") & total_paired < 8))
```
...
```{r}
# Counts for the plot
# utr3_counts <- list(nrow(dplyr::filter(utr3_clusters.df, Experiment == "STAU1" & !str_detect(cluster, "ID"))), nrow(paris.utr3.df))
