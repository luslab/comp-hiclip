...
```{r}
create_empty_grl <- function(mcol.ls, datatype.ls) {
    
  names(datatype.ls) <- mcol.ls
  df <- do.call(data.frame, datatype.ls)
  ...
  utr3.dt <- lapply(1:length(duplexes.grl), function(i) {
    ...
    if(length(unique(subjectHits(ol))) == 1) {
      ...
      if ((length(sel.ol) == 0)) {
        ...
        print(x)
        utr3 <- create_empty_grl(mcol.ls, datatype.ls)
        # utr3$tx_name <-NA
        names(utr3) <- NULL
...
```{r}
utr3_paris.dt <- data.table(dplyr::filter(utr3_clusters.df, Experiment == "PARIS"))
utr3_stau1.dt <- data.table(dplyr::filter(utr3_clusters.df, Experiment == "STAU1")) #  & detection_type == "Proximity ligation")
...
```{r}
mcol.ls = c('exon_id','exon_name','exon_rank') 
datatype.ls = list(integer(),character(),integer())
```
