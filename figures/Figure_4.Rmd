...
trim_duplex_overhangs <- function(clusters.df) {
  ...
  clusters.df <- clusters.df %>%
    rowwise() %>%
    mutate(L_genomic_duplex_start = L_genomic_start + L_duplex_start - 1,
           L_genomic_duplex_end= L_genomic_start + L_duplex_end - 1, 
           R_genomic_duplex_start = R_genomic_start + R_duplex_start - 1,
           R_genomic_duplex_end = R_genomic_start + R_duplex_end - 1) %>%
    ungroup()
...
predicted_stau1.utr3.df <- utr3_clusters.df %>%
  dplyr::filter(detection_type == "Prediction")
...
paired_metric.gg
# gsave(paste0(plots.dir, "/paired_metric.pdf"), paired_metric.gg, dpi = 300, width = 5, height = 4)
```

# Fig 4 G: Number of bulges
