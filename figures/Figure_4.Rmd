...
stau1.dt <- stau1.dt %>%
  mutate(type = case_when(L_gene_id == R_gene_id & sample_set != "stau1_nonhybrid" ~ "Intra-transcript",
                          L_gene_id == R_gene_id & sample_set == "stau1_nonhybrid" ~ "Intra-transcript",
                          L_gene_id != R_gene_id & L_seqnames == R_seqnames & sample_set == "stau1_nonhybrid" ~ "Intra-transcript",
                          TRUE ~ "Inter-transcript"))
```


```{r}
intra_stau1.dt <- test.dt %>%
...
paired_res.df$Experiment <- factor(paired_res.df$Experiment, levels = c("STAU1", "PARIS"))
au_pairs.gg <- ggplot(paired_res.df, aes(x = Experiment, y = AU_percentage, fill = condition)) + 
  geom_violin() +
  #scale_x_log10() + annotation_logticks() +
  scale_fill_manual(values = condition_colours) +
  ...
  theme_minimal_grid() + 
  theme(legend.position = "top") +
  ylab("A:U and U:A pairs (%) ") +
  # geom_vline(data=au_median, aes(xintercept=Median, color=Experiment),
  #            linetype="dashed") +
  facet_grid(cols = vars(span_group))

au_pairs.gg
# ggsave(paste0(plots.dir, "/AU_bp_content_violin.pdf"), au_pairs.gg, dpi = 300, width = 7, height = 6)
```
