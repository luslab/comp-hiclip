...
```{r}
library(gprofiler2)
stau.genes.dt <- unique(utr3.duplexes.dt[, .(L_gene_id, L_gene_name, metabolism_cluster)])
stau.genes.dt[, ensembl := tstrsplit(L_gene_id, "\\.")[[1]]]

gostres <- gost(query = list(A = stau.genes.dt[metabolism_cluster == "A"]$ensembl,
                             C = stau.genes.dt[metabolism_cluster == "C"]$ensembl), 
                organism = "hsapiens", 
                ordered_query = FALSE, 
                multi_query = TRUE, 
                significant = TRUE, 
                ...
                # custom_bg = stau.genes.dt$ensembl,
                # custom_bg = gene.counts.dt[stau %in% c("STAU1", "PARIS", "None\n(Expression\nMatched)")]$ensg, 
...
```{r}
duplex.info.dt <- fread("~/Dropbox (The Francis Crick)/comp_hiclip/stau1_atlas/stau1_duplex_info.tsv.gz")
duplex.info.dt <- duplex.info.dt[detection_type != "Predicted"]
gene.info.dt <- duplex.info.dt[, mean(AU_percentage), by = L_gene_id]
setnames(gene.info.dt, "V1", "mean_AU_percentage")
gene.info.dt[, ensg := tstrsplit(L_gene_id, "\\.")[[1]]]

gene.info.dt[ensg %in% rownames(annot.df)[annot.df$cluster == "A"], metabolism_cluster := "A"]
gene.info.dt[ensg %in% rownames(annot.df)[annot.df$cluster == "B"], metabolism_cluster := "B"]
gene.info.dt[ensg %in% rownames(annot.df)[annot.df$cluster == "C"], metabolism_cluster := "C"]
gene.info.dt <- gene.info.dt[!is.na(metabolism_cluster)]

ggplot(gene.info.dt, aes(x = metabolism_cluster, y = mean_AU_percentage, fill = metabolism_cluster)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) +
  scale_fill_manual(values = metabolism_colours) +
  theme_minimal_grid() + theme(legend.position = "none") +
  labs(x = "",
       y = "Mean AU percentage",
       fill = "")
```
