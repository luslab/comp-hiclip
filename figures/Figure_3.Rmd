...
```
```{r}
dplyr::filter(stau1.hybrids.dt, sample == "stau1_nonhybrid" & !str_detect(cluster, "C"))
```
...
```{r}
# clusters composed of predicted structures only
stau1.cluster.hybrids_summary.df %>%
  dplyr::filter(str_detect(id, "C") & sample_set == "stau1_nonhybrid")
```
```{r}
# clusters composed of predicted + experimental hybrids
stau1.cluster.hybrids_summary.df %>%
  dplyr::filter(str_detect(id, "C") & str_detect(sample_set, ",stau1_nonhybrid"))
...
***
> Duplexes in PIN1 3'UTR

```{r}
pin1_stau1.dt <- stau1.dt %>%
  dplyr::filter(L_gene_name == "PIN1")

pin1_stau1.dt$q <- 0
pin1_stau1.bp.dt <- pin1_stau1.dt[, .(L_genomic_seqnames, L_genomic_start, L_genomic_end, R_genomic_start, R_genomic_end, q)]

# fwrite(pin1_stau1.bp.dt, sep = "\t", col.names = FALSE, file ="~/Dropbox (The Francis Crick)/comp_hiclip/short_range/sept_2021/pin1_structures.bp")

dplyr::filter(x, name %in% "ID10276")
```
...
```{r}
short_range.df %>% dplyr::filter(name %in% short_range_overlapped.id.ls & hairpin_loop <=11)
```
