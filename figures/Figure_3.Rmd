...
```{r}
# cluster.duplexes.dt <- fread("~/Dropbox (The Francis Crick)/comp_hiclip/short_range/xl_plus_100/sept_2021/short_range_lmin_rnaduplex_structures.tsv.gz")
```
...
m_motif_max9bp <- list(nrow(cluster.duplexes.dt) - nrow(cluster.duplexes.dt[cluster.duplexes.dt$total_paired >= 8,]))
...
types_colours <- c("No M motif" = "#216b77", "M motif duplexes >= 8 bp" = "#31a1b3", "M motif duplexes < 8 bp" = "#7DCEA0")
...
```{r}
mfe <- fread("~/Desktop/short_range_duplexes.mfe.tsv.gz")
mfe <- distinct(mfe)
mfe <- dplyr::filter(mfe, !(str_detect(L_sequence, "N") | str_detect(R_sequence, "N")))

cluster.duplexes.dt <- left_join(cluster.duplexes.dt, dplyr::select(mfe, name, mean_shuffled_mfe), by = "name")
```


```{r}
cluster.duplexes.dt$mean_shuffled_mfe <- as.numeric(cluster.duplexes.dt$mean_shuffled_mfe)
cluster.duplexes.dt$mfe <- as.numeric(cluster.duplexes.dt$mfe)
...
```{r}
# Density plot
mfe.df <- pivot_longer(cluster.duplexes.dt, cols = c(mfe, mean_shuffled_mfe), names_to = "Sample", values_to = "MFE")

mfe.df <- mfe.df %>%
  mutate(Sample = case_when((Sample == "mfe") ~ "STAU1",
                            (Sample == "mean_shuffled_mfe") ~ "Shuffled control"))

mfe_median <- cluster.duplexes.dt %>%
...
```{r}
bp_colours <- c("M motif duplexes >= 8 bp" = "#31a1b3", "M motif duplexes < 8 bp" = "#7DCEA0")
...
mfe.dens.gg <- ggplot(mfe.df, aes(x = MFE, linetype = Sample, color = structure_type)) + 
  ...
  geom_vline(data=mfe_median, aes(xintercept=Median),
             linetype="dashed", color = "#31a1b3" )
...
```{r}
# Add stats:
stau1.mfe.ls <- dplyr::filter(cluster.duplexes.dt, structure_type == "M motif duplexes >= 8 bp")$mfe
stau1.shuffled.mfe.ls <- dplyr::filter(cluster.duplexes.dt, structure_type == "M motif duplexes >= 8 bp")$mean_shuffled_mfe
wilcox.test(stau1.mfe.ls, stau1.shuffled.mfe.ls)


mfe.ls <- dplyr::filter(cluster.duplexes.dt, structure_type == "M motif duplexes < 8 bp")$mfe
shuffled.mfe.ls <- dplyr::filter(cluster.duplexes.dt, structure_type == "M motif duplexes < 8 bp")$mean_shuffled_mfe
wilcox.test(mfe.ls, shuffled.mfe.ls)
```

#### Normalise to number of bp
...
# Density plot
mfe.df <- pivot_longer(cluster.duplexes.dt, cols = c(normalised_mfe, normalised_mean_shuffled_mfe), names_to = "Sample", values_to = "MFE")
...
mfe.dens2.gg <- ggplot(mfe.df, aes(x = MFE, linetype = Sample, color = structure_type)) + 
  geom_density() + # color = c("#31a1b3", "#5ec3d3")
  # annotate("text", label = "p < 2.2e-16", x = -1, y = 1.7) +
  scale_linetype_manual(values=c("longdash", "solid")) +
  scale_color_manual(values = bp_colours) +
...
# Plot
hairpin.gg <- ggplot(cluster.duplexes.dt, aes(x = hairpin_loop)) + 
  #geom_histogram(bins = 45, color="black", fill = "#DEEBF7") +
  geom_density(color = "#31a1b3", bw = 0.1) +
  theme_minimal_grid() +
  geom_vline(data=hairpin_median, aes(xintercept=Median), color = "grey",
...
hairpin.gg
# ggsave(paste0(plots.dir,"/",prefix,"_hairpin_lengths.pdf"), hairpin.gg, width = 5, height = 4)
```
...
## Fig 3 G: Breakdown of the atlas
...
```{r}
nrow(dplyr::filter(stau1.hybrids.dt, sample == "stau1_nonhybrid" & str_detect(cluster, "ID")))
```
...
# Remove predicted duplexes with less than 8 bp
stau1.dt <- stau1.dt %>%
  dplyr::filter(!(str_detect(cluster, "ID") & !(name %in% cluster.duplexes.dt$name)))
...
stau1.dt$Experiment <- "STAU1"
stau1.dt <- left_join(stau1.dt, dplyr::select(stau1.samples.df, Experiment, id, sample_set), by = c("Experiment", c("name" = "id")))
```
...
source_counts.df <- stau1.dt %>%
  ...
  dplyr::select(name, Experiment, sample_set) %>%
  distinct() %>%
  group_by(sample_set) %>%
  summarise(counts = n()) 
...
```{r}
# Obtain IDs of overlapped spredicted short-range structures
```
...
Candidates: ID11592, ID12707, ID16902
...
```{r}
hur.prob.file <- prob.files[str_detect(prob.files, "ELAV")]
hur.shuff.file <- shuff.files[str_detect(prob.files, "ELAV")]
```
...
get_metaprofile_dataframe <- function(prob.file, shuff.file = NULL, shuffled = FALSE) {
  ...
  rbp.prob.df <- data.table::fread(prob.file, skip = 1, data.table = FALSE)
...
```{r}
hur.df <- get_metaprofile_dataframe(hur.prob.file, hur.shuff.file, shuffled = TRUE)
```
