...
plot_pie_chart <- function(data.df, column, group = NULL) {
  ...
  pie <- ggplot(data_counts.df, aes(x="", y=counts, fill={{column}})) +
    ...
    ggrepel::geom_label_repel(aes(label = counts), size = 2.5, show.legend = FALSE, position = position_stack(vjust = .3)) +
    ggtitle(paste0("Clusters 1, 3, 4: ", "n = ", sum(data_counts.df$counts),""))+
    guides(fill = guide_legend(title = "Structure type")) +
    theme_void() # remove background, grid, numeric labels
  
  return(list(plot = pie, data = data_counts.df))
}
...
```{r}
# ggsave(paste0(data.dir, "/peak_regions_vertical.pdf"), peak_regions.gg, width = 8, height = 3)
```
...
stau1.prob.df <- data.table::fread(prob.file, skip = 1, data.table = FALSE)
...
stau1.profile.gg <- append_layers(stau1.profile.gg, annotate("rect", xmin = 15, xmax = 75, ymin = -Inf, ymax = Inf, 
...
#ggsave(paste0(data.dir,"/",prefix,"_metaprofile_paired.pdf"), stau1.profile.gg, width = 4, height = 4)
...
```{r}
set.seed(123)
silhouette.gg <- fviz_nbclust(prob_downstream.df, kmeans, method="silhouette")

silhouette.gg

# ggsave(paste0(data.dir,"/silhouette_2k.pdf"), silhouette.gg, dpi = 300, width = 4, height = 4)
```
...
stau1.prob.df$cluster <- plyr::mapvalues(stau1.prob.df$cluster, from=c(2, 1), to=c(0, 1))
...
silhouette_cl1.gg
# ggsave(paste0(data.dir,"/silhouette_cluster1.pdf"), silhouette_cl1.gg, dpi = 300, width = 4, height = 4)
```
...
stopifnot(nrow(stau1.prob.df) == nrow(clusters.df)) # Chekc same size as original prob df
```

```{r}
col_pal <- brewer.pal(4, "Dark2")
plot_cluster_heatmap(clusters.df, plot.title = prob.name, plot.name = paste0(data.dir,"/",prefix,"_kmeans_minus50_paired_4clusters.png"))
```
...
clusters.gg <- append_layers(clusters.gg, annotate("rect", xmin = 15, xmax = 75, ymin = -Inf, ymax = Inf, 
           alpha = 0.5, fill = "#DEEBF7"), position = "bottom")
...
Filter the forgi annotation data for peaks (IDs) in clusters 1, 2 and 3.


```{r}
clust_prob.df <- clusters.df %>%
  dplyr::filter(cluster == 1 | cluster == 2 | cluster == 3)

forgi.df <- semi_join(forgi.df, clust_prob.df, by = "id")
rnafold.df <- semi_join(rnafold.df, clust_prob.df, by = "id")
```

There are `r length(unique(forgi.df$id))` peaks in clusters 1,2,3.


#### Calculate mean distance from xl site for each cluster
...
xl_dist.df <- forgi.df %>%
  ...
  dplyr::select(id, L_start) %>%
  distinct() %>%
  mutate(distance = L_start - xl + 1)

xl_dist.df <- left_join(xl_dist.df, dplyr::select(clust_prob.df, id, cluster), by = "id")
xl_dist_mean.df <- xl_dist.df %>%
  group_by(cluster) %>%
...
# There are some null multiloop structures that contain junctions, extract them
junctions_first_stem.df <- multi_sl.df %>%
  dplyr::filter(stem_position == "first" & (element == "h1" | element == "h2"))

junctions_second_stem.df <- multi_sl.df %>%
  dplyr::filter(stem_position == "second" & (element == "h2"| element == "h3"))

junctions.df <- rbind(junctions_first_stem.df, junctions_second_stem.df)

# remove junction-containing structures
multi_sl.df <- anti_join(multi_sl.df, junctions.df, by = "id")
...
length(unique(multi_sl.df$peak_id))
```



```{r}
# Re-annotate duplex types to account for the null multi-loops identified above
forgi.df <- forgi.df %>%
  mutate(duplex_type = case_when(duplex_type == "Stem and xl unpaired\n(no multiloops, 1st or all multiloops are null)" & 
                                   id %in% junctions_first_stem.df$id ~ "Stem and xl unpaired\n(multiloops)",
                                 TRUE ~ duplex_type)) 
...
```{r}
multi_sl.df <- multi_sl.df %>%
  dplyr::select(colnames(first_sl.df))
...
length(unique(all_sl.df$id))
# Number of peaks with stem-loops
length(unique(all_sl.df$peak_id))
```

> Classification of structures

The *Stem and xl unpaired* were further characterised. 
...
```{r}
# plot_stacked_barchart(forgi.df, structure_definition)$data
```


```{r}
z.df <- forgi.df %>%
  mutate(structure_definition = case_when(structure_definition == "Valid putative stem-loop structures" & id %in% all_sl.df$peak_id ~ "Peaks with predicted stem-loop duplexes >= 10 bp",
                                          structure_definition == "Valid putative stem-loop structures" & !(id %in% all_sl.df$peak_id) ~ "Peaks with predicted stem-loop duplexes < 10 bp",
                                          structure_definition == "No structure or the first stem\nspans a multi-loop" ~ "Peaks with no predicted stem-loop duplexes"))

pie.gg <- plot_pie_chart(z.df, structure_definition)
pie.gg
```
...
```{r}
#ggplot2::ggsave(filename = "pie", plot = pie.gg, device = "pdf")
ggsave(paste0(data.dir,"/",prefix,"pie.pdf"), pie.gg, width = 4, height = 4)
```
```{r}
plot_stacked_barchart(z.df, structure_definition)$data
```
### Types of predicted structures


```{r}
# how many peaks with 2 fused stem-loops?

two_stems.df <- all_sl.df %>%
  dplyr::filter(str_detect(id, "_2")) 

fused_stems.df <- all_sl.df %>%
  dplyr::filter(peak_id %in% unique(two_stems.df$peak_id)) %>%
  dplyr::select(id, peak_id) %>%
  distinct()

fused_stems.df <- fused_stems.df %>% dplyr::arrange(id)
...
```{r}
# how many peaks with perfect stem loops?

all_sl.df <- all_sl.df %>%
  group_by(id) %>%
  mutate(stem_type = case_when(any(element_type == "i") ~ "Stem with bulges",
                               TRUE ~ "Perfect stem")) %>%
  ungroup()

all_sl.df %>%
  dplyr::select(id, peak_id, stem_type) %>%
  distinct() %>%
  group_by(stem_type) %>%
  count(vars = "id")
```
...
# Perfect stem 
perfect_stems.df  <- all_sl.df %>%
  dplyr::filter(stem_type == "Perfect stem")

perfect_stems.df <- perfect_stems.df %>%
  dplyr::filter(element_type == "s" & L_width == 12 & hairpin_length <= 5)
...
```{r}
hairpin_median <- hairpin.df %>%
  ungroup() %>%
  dplyr::summarize(Median = median(hairpin_length, na.rm=TRUE))
hairpin_median
```


```{r}
max(hairpin.df$hairpin_length)
```

```{r}
quantile(hairpin.df$hairpin_length, c(.25, .50, .75, 1))
```
...
h0.gg <- ggplot(hairpin.df, aes(x=hairpin_length)) + 
  ...
  theme_minimal() +
  geom_vline(data=hairpin_median, aes(xintercept=Median),
             linetype="dashed") +
  scale_x_log10(breaks=c(3,5,7, 10,30)) + 
...
```{r}
# Export bed file
forgi.reformated.gc.dt <- data.table(forgi.reformated.gc.df)

export_genomic_bed(forgi.reformated.gc.dt, paste0(data.dir, "/predicted_duplexes.bed"))

```

```{r}
dplyr::filter(forgi.reformated.gc.df, name == "ID17880")
```
```{r}
dplyr::filter(arms.df, id == "ID17880")

# chr8	26656048	26656077	ID17880||||	0	+	26656048	26656077	0	2	12,12	0,17
...
olap_counts <- c(Predicted_stem_loop_duplexes_min_10_bp = nonhyb_count-nonhyb_ol_count, hiCLIP_duplexes = hyb_clusters_count-nonhyb_ol_count, "Predicted_stem_loop_duplexes_min_10_bp&hiCLIP_duplexes" = nonhyb_ol_count)

#pdf(file=paste0(data.dir,"/overlap.pdf"), onefile=FALSE) 
...
upset(fromExpression(olap_counts),
      ...
      point.size = 2.8,
      line.size = 1,
      mainbar.y.label = "Intersection cont",
...
```{r}
# nonhyb_count <- nrow(count(all_sl.df, id))
linker_clusters_count <- nrow(linker.collapsed.dt)
nolinker_clusters_count <- nrow(nolinker.collapsed.dt)
...
```{r}
# forgi
fwrite(all_sl.df, paste0(data.dir,"/stau1_nonhybrid.processed.forgi.tsv.gz"), sep = "\t", col.names = TRUE)

# hybrids
write_tsv(forgi.reformated.gc.df, paste0(data.dir,"/stau1_nonhybrid.gc.txt"), quote = FALSE)
write_tsv(forgi.reformated.tx.df, paste0(data.dir,"/stau1_nonhybrid.tx.txt"), quote = FALSE)
```


# Supplementary figure / Other RBPs

The median number of bp per stem-loop structure is `r median(all_sl.df$total_paired)`.
The mean is `r mean(all_sl.df$total_paired)`bp.
The longest stem is `r max(all_sl.df$total_paired)` bp long.

The following plots show the distributions of number of bp per structure
...
```{r}
tot.gg <- ggplot(dplyr::distinct(all_sl.df, id, total_paired), aes(x=total_paired)) + 
  geom_histogram(binwidth=1, color="black", fill = "white") +
  ylim(0, 300)+
  xlim(0, 40)+
  theme_minimal()+
  ggtitle("Total base-pairs per predicted stem-loop (before cutting the stem) ")
tot.gg
```
