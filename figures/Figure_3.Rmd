...
Symmetrical (vertical plane) | Symmetric
Symmetrical (horizontal plane) | Symmetric
...
```{r}
# # Count the number of internal loops on either side of the arm
# utr3_forgi.df <- utr3_forgi.df %>%
#   replace(is.na(.), 0)
# 
# left_iloop_count.df <- utr3_forgi.df %>%
#   group_by(Experiment, id) %>%
#   dplyr::filter(element_type == "i" & L_start > 0) %>%
#   count() %>%
#   dplyr::rename(left_iloop_count = n)
# 
# right_iloop_count.df <- utr3_forgi.df %>%
#   group_by(Experiment, id) %>%
...
# 
# 
# all_sl.df <- left_join(utr3_forgi.df, left_iloop_count.df, by = c("Experiment","id"))
# all_sl.df <- left_join(all_sl.df, right_iloop_count.df, by = c("Experiment","id"))
```
...
test <- utr3_forgi.df %>%
  ...
  dplyr::filter(element_type == "i") %>%
  summarize(L_i_widths = paste(sort((L_width)),collapse=","),
            R_i_widths = paste(sort((R_width)),collapse=",")) %>%
  ungroup()
