...
stau1.span.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$duplex_span
paris.span.ls <- dplyr::filter(utr3_clusters.df, Experiment == "PARIS")$duplex_span

# Kolmogorov-Smirnov test
span.pval <- ks.test(stau1.span.ls, paris.span.ls, exact = F)$p.val
span.pval
```
...
duplex_span.gg <- ggplot(utr3_clusters.df, aes(x = duplex_span, color = Experiment)) + 
  ...
  geom_vline(data=span_median, aes(xintercept=Median, color=Experiment),
             linetype="dashed") +
  annotate("text", label = paste0("p = ", span.pval), x = 100, y = 0.65)
...
mixmdl <- normalmixEM(log10(utr3_clusters.df$duplex_span), k = 2)
ggplot() +
...
```{r}
# dgof::ks.test(stau1.span.ls, paris.span.ls, simulate = TRUE, B=10000)
#KSgeneral::disc_ks_test(stau1.span.ls, ecdf(paris.span.ls), exact = T)
#Matching::ks.boot(stau1.span.ls, paris.span.ls, nboots=1000)
```
...
utr3_clusters.df <- utr3_clusters.df %>%
  ...
  mutate(normalised_mfe_zscore = (normalised_mfe - normalised_mean_shuffled_mfe)/normalised_sd_shuffled_mfe) %>%
  ungroup()
```

```{r}
#mean(utr3_clusters.df$normalised_mfe_zscore)
#mean(utr3_clusters.df$mfe_zscore < -1.96)
#pval = pnorm(utr3_clusters.df$normalised_mfe_zscore)

#nrow(dplyr::filter(utr3_clusters.df, normalised_mfe_zscore < -2))/nrow(utr3_clusters.df)
#stau1.zscore.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$normalised_mfe_zscore
#paris.zscore.ls <- dplyr::filter(utr3_clusters.df, Experiment == "PARIS")$normalised_mfe_zscore

#length(stau1.zscore.ls[stau1.zscore.ls < -2])/length(stau1.zscore.ls)
#length(paris.zscore.ls[paris.zscore.ls < -2])/length(paris.zscore.ls)
```
...
stau1.shuffled.mfe.ls <- dplyr::filter(utr3_clusters.df, Experiment == "STAU1")$normalised_shuffled_mfe
...
paris.shuffled.mfe.ls <- dplyr::filter(utr3_clusters.df, Experiment == "PARIS")$normalised_shuffled_mfe
...
mfe.dens.gg <- ggplot(utr3_clusters_mfe.df, aes(x = MFE, linetype = Sample, color = Experiment)) + 
  ...
  ylab("Density") +
  xlab("Normalised hybridisation energy (kcal/mol)") 
  #xlab("MFE (kcal/mol)") +
  #ggtitle("3'UTR-3'UTR clusters binding energy") 

mfe.dens.gg
# ggsave(paste0(data.dir, "mfe_dens.pdf"), mfe.dens.gg, dpi = 300, width = 4, height = 6)
```
...
```{r}
paired_metric.gg <- ggplot(utr3_clusters.df, aes(x=paired_percentage, fill = Experiment)) + 
  ...
  theme(legend.position = "top") +
  ylab("Density") +
  xlab("Percentage of paired residues") +
  ...
  #annotate("text", x = 90, y = 1, paste0("p = ", paired_ratio.pval))
...
bulges.gg <- ggplot(utr3_clusters.df,
  ...
  geom_label(data = utr3_counts.df, aes(label=paste0("n = ",counts), y = -1.5), label.padding = unit(0.25, "lines"),
            position=position_dodge(width=1.0), inherit.aes = TRUE,  fill = "white", size = 6)
bulges.gg
# ggsave(paste0(data.dir, "bulge_count.pdf"), bulges.gg, dpi = 300, width = 4, height = 6)
```
## Fig 3 H
...
bulge_s.gg <- ggplot(median_bulge_spacing.df,
  ...
  #           position=position_dodge(width=1.0), inherit.aes = TRUE,  fill = "white", size = 6)
bulge_s.gg
ggsave(paste0(data.dir, "bulge_spacing.pdf"), bulge_s.gg, dpi = 300, width = 4, height = 6)
```
