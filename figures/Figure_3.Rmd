...
tardp.data.df <- get_metaprofile_dataframe(tardp.prob.file, hur.shuff.file, shuffled = TRUE)
```
...
short_range.df <- short_range.df %>%
  ...
  mutate(L_duplex_start= paired_regions$l_start, 
         L_duplex_end = paired_regions$l_end - paired_regions$l_start,
         R_duplex_start= paired_regions$r_start, 
         R_duplex_end = paired_regions$r_end - paired_regions$r_start) %>%
  ungroup() %>%
  dplyr::select(-paired_regions)

short_range.df <- short_range.df %>%
  rowwise() %>%
  mutate(L_genomic_start = case_when(L_genomic_strand == "+" ~ L_genomic_start + L_duplex_start - 1,
                                    TRUE ~ L_genomic_end - L_duplex_end + 1),
         L_genomic_end = case_when(L_genomic_strand == "+" ~ L_genomic_start + L_duplex_end - 1,
                                  TRUE ~ L_genomic_end - L_duplex_start + 1),
         R_genomic_start = case_when(L_genomic_strand == "+" ~ R_genomic_start + R_duplex_start - 1,
                                    TRUE ~ R_genomic_end - R_duplex_end + 1),
         R_genomic_end = case_when(L_genomic_strand == "+" ~ R_genomic_start + R_duplex_end - 1,
...
short_range.df <- left_join(short_range.df, dplyr::select(mfe, name, mean_shuffled_mfe), by = "name")
```
