...
plot_stacked_barchart <- function(data.df, column, group=NULL, simplify=TRUE) {
  ...
  bar = ggplot() + geom_bar(aes(y = percentage, x = "", fill = category), data = data_counts.df,
    ...
    geom_text(data = data_counts.df, aes(x = "", y = pos, label=paste0(round(percentage, 1),"%")), size = 3) + # label=paste0(round(percentage, 2),"%"))
...
plot_stacked_barchart(paris.dt, column = type, simplify = FALSE)
```
...
```{r}
plot_nucleotide_frequency(utr3_clusters.df)$plot +
...
```{r}
# Get the complete set of internal loop wisdths for L and R arm 
...
symmetry.df <- utr3_clusters.df %>%
  rowwise() %>%
  mutate(symmetry_type = case_when(stem_type == "Interrupted stem" & L_i_widths_sorted == R_i_widths_sorted ~ "Symmetric",
                                   stem_type == "Interrupted stem" & L_i_widths_sorted != R_i_widths_sorted ~ "Asymmetric",
...
plot_stacked_barchart(symmetry.df, column = symmetry_type, group = Experiment, simplify = FALSE) +
  scale_fill_brewer(palette = "Blues") +
  theme_minimal_grid()
```
```{r}
...
```
```{r}
dplyr::filter(symmetry2.df, name == "C001-NBPF3:ENSG00000142794.18-NBPF3:ENSG00000142794.18")
```

```{r}
dplyr::filter(utr3_forgi.df, id == "C001-NBPF3:ENSG00000142794.18-NBPF3:ENSG00000142794.18")
