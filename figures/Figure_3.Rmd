...
```{r}
data.dir <- "/Users/iosubi/Dropbox (The Francis Crick)/comp_hiclip/short_range/xl_plus_100/july_2021"
plots.dir <- "/Users/iosubi/Dropbox (The Francis Crick)/comp_hiclip/plots/figure_3"
...
```{r}
prob.files <- list.files(paste0(data.dir,"/data"), pattern = "rnaplfold_prob.tsv.gz", full.names = TRUE)
shuff.files <- list.files(paste0(data.dir,"/data"), pattern = "rnaplfold.shuffled_prob.tsv.gz", full.names = TRUE)
...
***
> Short-range duplex data

```{r}
short_range.dt <- fread(paste0(data.dir, "/results/short_range_lmin_rnaduplex_structures.tsv.gz"))
```
...
```{r}
bed.files <- list.files(path = paste0(data.dir,"/data"), pattern = "annot.bed.gz", full.names = TRUE)
...
There are `r length(unique(short_range.dt$name))` peaks in clusters 1,2,3.
...
# Adjust genomic coordinates to only the paired region
short_range.dt <- short_range.dt %>%
  rowwise() %>%
...
```{r}
short_range.dt <- dplyr::filter(short_range.dt, loop != 0)
```
...
```{r}
hairpin_median <- short_range.dt %>%
  dplyr::summarize(Median = median(hairpin_loop, na.rm = TRUE))
hairpin_median

# Plot
h0.gg <- ggplot(short_range.dt, aes(x = hairpin_loop)) + 
...
```{r}
quantile(short_range.dt$hairpin_loop, c(.25, .50, .75, 1))
```
...
# fwrite(short_range.reformated.gc.df, paste0(data.dir, "/results/short_range_duplexes.tsv.gz", sep = "\t"))
...
```{r}
nonhyb_count <- nrow(short_range.dt)
...
```{r}
semi_join(short_range.dt, nonhyb_overlapped.dt, by = "name")
...
```
```{r}
ex1.df <- dplyr::filter(short_range.dt, name == "ID10081")
```
...
```{r}
anti_join(short_range.dt, nonhyb_overlapped.dt, by = "name")
...
```{r}
ex2.df <- dplyr::filter(short_range.dt, name == "ID10083")
```
...
short_range.dt <- left_join(short_range.dt, dplyr::select(short_range.reformated.gc.df, name, L_sequence, R_sequence))
```

```{r}
x <- get_stem_loop_sequence(as.data.table(short_range.reformated.gc.df), fa.dt)
short_range.dt <- left_join(short_range.dt, dplyr::select(x, name, stem_loop_sequence))
```

```{r}
dplyr::filter(short_range.dt, name == "ID10083" | name == "ID10081")
```
...
```{r}
m_motif_structures <- list(nrow(short_range.dt))
names(m_motif_structures) <- "M motif duplexes (clusters 1, 2, 3)"
m_motif_min10bp <- list(nrow(short_range.dt[short_range.dt$total_paired >= 10,]))
names(m_motif_min10bp) <-"M motif duplexes >= 10 bp"

m_motif_max9bp <- list(nrow(short_range.dt) - nrow(short_range.dt[short_range.dt$total_paired >= 10,]))
names(m_motif_max9bp) <- "M motif duplexes < 10 bp"

no_m_motif <- list(nrow(stau1.prob.df) - nrow(short_range.dt))
...
```{r}
total_paired_median <- short_range.dt %>%
  dplyr::summarize(Median = median(total_paired, na.rm = TRUE))
total_paired_median

# Plot
s.gg <- ggplot(short_range.dt, aes(x=total_paired)) + 
  #geom_histogram(bins = 45, color="black", fill = "#DEEBF7") +
...
#   
#   coord.dt <- short_range.dt
#   coord.dt$q <- cut(short_range.dt$score, 
