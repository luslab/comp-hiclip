...
plot_cluster_heatmap(clusters.df, plot.title = "STAU1", plot.name = paste0(plots.dir,"/",prefix,"_kmeans_minus50_paired_4clusters.png"))
...
# cluster.duplexes.dt <- fread("~/Dropbox (The Francis Crick)/comp_hiclip/short_range/sept_2021/short_range_lmin_rnaduplex_structures.tsv.gz") # The above chunk takes long to run, so you can load the data directly
```
...
cluster.duplexes.dt <- cluster.duplexes.dt %>%
  dplyr::filter(name %in% short_range.df$name) # remove loops = 0 as they suggest homo-heterodimers rather than stem loops
```
...
```{r}
# Work with the annotated structures
short_range.df <- short_range.df %>%
  rowwise() %>%
...
# Total number of clustered hybrids (without the unclustered M motif structures)
nrow(dplyr::filter(stau1.hybrids.dt, str_detect(cluster, "C")))
```
...
source_counts.df # input for upset plot
stopifnot(nrow(stau1.dt) == sum(source_counts.df$counts))
```
...
dplyr::filter(stau1.dt, sample_set == "stau1_nonhybrid") # M motif unclustered
dplyr::filter(stau1.dt, sample_set == "stau1_nonhybrid" & str_detect(cluster, "C")) # M motif structures clustering together
...
nrow(dplyr::filter(stau1.hybrids.dt, sample == "stau1_nonhybrid" & !str_detect(cluster, "C")))


# Number of hybrids from M motif structures (sum of clustered + unclustered)
nrow(dplyr::filter(stau1.hybrids.dt, sample == "stau1_nonhybrid"))
...
***
> Duplexes in ARF1 3'UTR

```{r}
arf1_stau1.dt <- stau1.dt %>%
  dplyr::filter(L_gene_name == "ARF1")
...
Are the loops of overlapped derived duplexes larger than the loops of non-overlapped derived duplexes?
...
Number of hybrids supporting duplexes
