...
```{r}
paired_metric.gg <- ggplot(utr3_clusters.df, aes(x=paired_to_total_ratio, color = Experiment)) + 
  geom_density() +
  scale_color_tableau() +
  theme_minimal_grid() + 
  #facet_grid(rows = vars(Experiment)) +
...
```
```{r}
dplyr::filter(utr3_clusters.df, Experiment == "PARIS")
```
```{r}
dplyr::filter(unpaired.df, Experiment == "PARIS" & id == "C013-AL031428.1_LUZP1:ENSG00000169641.13_ENSG00000240553.1-AL031428.1_LUZP1:ENSG00000169641.13_ENSG00000240553.1")
```
```{r}
unpaired.df <- get_unpaired_total(utr3_forgi.df, group = Experiment)
