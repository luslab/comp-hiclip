...
prob.df$cluster <- plyr::mapvalues(prob.df$cluster, from=c(2, 1), to=c(0, 1))
prob.df <- dplyr::arrange(prob.df, cluster)
prob.df$cluster <- factor(prob.df$cluster, levels = c(0, 1))
```
...
stopifnot(nrow(prob.df) == nrow(clusters.df)) # Chekc same size as original prob df
```
...
pie.gg  
# ggsave(paste0(data.dir,"/pie.pdf"), pie.gg, dpi = 300, width = 6, height = 6 )
```
```{r}
