...
correct_iloop_positions <- function(stem_loop.df, reference_point = "h0") {
  ...
  # Position of iloop/bulge start relative to the hairpin can be used to correct this
  # !!!THIS FUNCTION MUST BE APPLIED ON SINGLE SELECTED STEM LOOP STRUCTURES (a single hairpin present)
  
  h0.df <- stem_loop.df %>%
    group_by(id) %>%
    dplyr::filter(element == reference_point) %>%
    mutate(h_start = L_start, h_end = L_end ) %>%
    dplyr::select(id, h_start, h_end) %>%
    ungroup() %>%
    distinct()

  stem_loop.df <- left_join(stem_loop.df, h0.df, by = "id")

  if (nrow(dplyr::filter(stem_loop.df, L_start > h_end)) == 0) {

    stem_loop.df <- stem_loop.df%>%
      dplyr::select(-h_start, -h_end)

  } else {

    stem_loop.df <- stem_loop.df %>%
      rowwise() %>%
      mutate(swap = case_when((element_type == "i") & (as.double(L_start) > h_end) ~  "yes", TRUE ~ "no"),
             R_start = case_when((element_type == "i") & (as.double(L_start) > h_end) ~ as.double(L_start),
                                 TRUE ~ as.double(R_start)),
             R_end = case_when((element_type == "i") & (as.double(L_start) > h_end) ~ as.double(L_end),
                               TRUE ~ as.double(R_end)),
             R_width = case_when((element_type == "i") & (as.double(L_start) > h_end) ~ as.double(L_width),
  ...
  stem_loop.df <- stem_loop.df %>%
    ...
    mutate(L_start = case_when(swap == "yes" ~ 0, 
        ...
        ungroup() %>%
        dplyr::select(-h_start, -h_end, -swap)
...
separate_stem_loops <- function(forgi.df) {
  ...
  first_multi.df <- correct_iloop_positions(first_multi.df, reference_point = "h0")
  ...
  second_multi.df <- correct_iloop_positions(second_multi.df, reference_point = "h1")
...
crop_stem <- function(data, idx, threshold = 20) {
  
  data.df <- data %>% 
    dplyr::filter(idx == id)
  data.df <- rowid_to_column(data.df, var = "position")
  
  stems.df <- data.df %>%
    dplyr::filter(element_type == "s") %>%
    mutate(stem_cumsum = cumsum(L_width))
  
  nonstems.df <- anti_join(data.df, stems.df, by = "position")
  nonstems.df$stem_cumsum <- 0
  nonstems.df$stem_cumsum_cut <- 0
  
  # update L_width so only the first 20 paired res are taken
  stems.df <- stems.df %>%
    mutate(L_width = case_when((stem_cumsum > threshold) ~ as.double(L_width) - (as.double(stem_cumsum) - threshold),
                               TRUE ~ as.double(L_width))) %>%
    mutate(stem_cumsum_cut = case_when((stem_cumsum > threshold) ~ threshold,
                                    TRUE ~ as.double(stem_cumsum))) %>%
    dplyr::filter(stem_cumsum_cut <= threshold)
  
  # rejoin
  remerged.df <- rbind(stems.df, nonstems.df)
  remerged.df <- dplyr::arrange(remerged.df, position)
  
  max_stem_cumsum <- stems.df %>%
    slice_max(stem_cumsum_cut, n=1)
  
  max_stem_cumsum <- min(max_stem_cumsum$position)
  # print(max_stem_cumsum)
  
  remerged.df <- remerged.df %>%
    dplyr::filter(position <= max_stem_cumsum) 
...
```{r}
txdb <- paste0(getwd(),"/gencode.v33.txdb.sqlite")
gencode.txdb <- AnnotationDbi::loadDb(txdb)
human.gtf <- paste0(getwd(), "/GRCh38.gencode_v33.tx.gtf.gz") # contains fasta_id used by Tosca
...
```{r}
bed.files <- list.files(getwd(),path = , pattern = "annot.bed.gz", full.names = TRUE)
...
```{r}
prob.files <- list.files(getwd(), pattern = "rnaplfold_prob.tsv.gz", full.names = TRUE)
shuff.files <- list.files(getwd(), pattern = "rnaplfold.shuffled_prob.tsv.gz", full.names = TRUE)
...
```{r}
forgi.df <- fread("stau1.forgi.tsv.gz", data.table = FALSE)
rnafold.df <- fread("stau1.rnafold.tsv.gz", data.table = FALSE)
...
plot_stacked_barchart(peak_regions.df, column = type)
```
...
```{r}
plot_cluster_heatmap(prob.df, plot.title = prob.name, plot.name = paste0(prefix,"_kmeans_minus50.png"))
```
...
```{r fig3b, fig.height = 9, fig.width = 2, fig.align = "center"}
clusters.gg
```
...
first_sl.df <- correct_iloop_positions(first_sl.df)
```
...
```{r}
h0.gg <- ggplot(all_sl.df, aes(x=hairpin_length)) + 
